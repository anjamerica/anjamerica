...
export default function Contact() {
  ...
  return (
    <div className="w-fit h-fit px-10 py-5 md:p-20 z-0  grid grid-cols-1 gap-8 md:grid-cols-2 bg-primary-green rounded-xl">
      <Loader />
      <div className="flex flex-col gap-2 md:gap-8">
        ...
        <div className="flex flex-col gap-2">
          ...
          <span className="font-thin text-sm text-[#141414]">
            9253 Centauri Ave <br /> Columbus, OH - 43240
          </span>
        </div>
        <div className="flex flex-col gap-2">
          <span className="text-[12px] font-semibold text-[#141414]">
      ...
      <div className="flex flex-col gap-2 w-full">
        <div className="flex flex-row justify-between gap-2">
          <div>
            <input
              ...
              onChange={(e) => handleInputChange("first_name", e.target.value.replace(/[^a-z]/gi, ''))}
            />
            <span className="text-xs text-red-600">{formError.name_err}</span>
          </div>
          <div>
            <input
            type="number"
            ...
            />
            <span className="text-xs text-red-600">{formError.number_err}</span>
          </div>
        </div>
        <div>
          <input
          ...
          />
          <span className="text-xs text-red-600">{formError.email_err}</span>
        </div>
        <div>
          <input
            className="form-input"
        ...
        <div>
          ...
          />
          <span className="text-xs text-red-600">{formError.message_err}</span>
        </div>
        <hr />
        <button
          className="font-medium mt-2 text-white bg-secondary-green px-4 py-3 hover:bg-[#66c7c1] transition-all   rounded-full text-center flex items-center justify-center text-xs w-full"
